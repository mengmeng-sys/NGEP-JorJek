...
const ROLE_COLORS = {
  PROFESSOR: { bg: "bg-orange-50 dark:bg-orange-900/20", text: "text-[#FF4F00]", border: "border-orange-100 dark:border-orange-900/30" },
  STUDENT: { bg: "bg-blue-50 dark:bg-blue-900/20", text: "text-blue-600 dark:text-blue-400", border: "border-blue-100 dark:border-blue-900/30" },
};
  ...
  if (users.length === 0) {
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-[11px] text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 font-medium">No top users yet</p>
        <p className="text-[10px] text-gray-300 dark:text-gray-600 dark:text-gray-400 dark:text-gray-500 mt-0.5">Start posting and earning karma!</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2.5">
      {users.slice(0, 5).map((user, index) => {
        ...
        const rankStyle = RANK_STYLES[index] || "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 dark:text-gray-500";

        return (
          <div
            key={user.id}
            onClick={() => handleUserClick(user)}
            className="group flex items-center gap-3 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800/50 dark:hover:bg-gray-800 transition-colors cursor-pointer"
          >
            {/* Rank Badge */}
            ...
            {/* Avatar */}
            <UserAvatar initials={initials} userId={user.id} size="md" gradient className="ring-2 ring-white dark:ring-gray-900 shadow-xs" avatarUrl={user.avatarUrl || user.avatar_url} />
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-[11px] text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 font-medium flex items-center gap-0.5">
