...
export function PostCard({ post, onToggleSave, onDelete, onEdit }) {
  ...
  return (
    <div
      ...
      className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl sm:rounded-2xl overflow-hidden shadow-xs mb-3.5 sm:mb-4 cursor-pointer hover:border-gray-300 dark:border-gray-700 transition-all"
    >
      <div className="p-4 sm:p-5">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <Link
              ...
              className="h-9 w-9 sm:h-10 sm:w-10 bg-[#111827] text-white font-black flex items-center justify-center rounded-full text-xs hover:ring-2 hover:ring-offset-2 transition-all shrink-0 overflow-hidden"
            >
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                <Link
                  to={`/user/${userProfileSlug}`}
                  onClick={(e) => e.stopPropagation()}
                  className="font-bold text-gray-900 dark:text-gray-100 text-xs sm:text-sm hover:text-[#FF4F00] transition-colors truncate max-w-32.5 sm:max-w-50"
                >
                  {post.author}
                </Link>

                <span className="bg-purple-50 dark:bg-purple-900/20 text-[#8B5CF6] text-[9px] sm:text-[10px] uppercase font-bold px-1.5 sm:px-2 py-0.5 rounded border border-purple-100 dark:border-purple-900/30 shrink-0">
                  {post.role || 'STUDENT'}
                </span>
              </div>
          ...
          <div className="relative shrink-0" ref={menuRef}>
            <button
              ...
              className="p-1 sm:p-1.5 text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300 dark:text-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-800 transition-colors cursor-pointer"
              title="More options"
            >
            ...
            {isMenuOpen && (
              <div
                onClick={(e) => e.stopPropagation()}
                className="absolute right-0 mt-1 w-44 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-lg py-1.5 z-30 animate-in fade-in zoom-in-95 duration-100"
              >
                {isOwner ? (
                  <>
                    <button
        ...
        {/* Post Title & Content */}
        <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100 mb-1.5 sm:mb-2 leading-snug hover:text-[#FF4F00] transition-colors wrap-break-words">
          {post.title}
        </h2>
        {post.content ? (
          <div className="relative mb-3">
            <p className={`text-xs sm:text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 leading-relaxed wrap-break-words ${isLongContent && !isContentExpanded ? 'line-clamp-3' : ''}`}>
              {post.content}
            </p>
            {isLongContent && (
              <button
                ...
                className="mt-1 inline-flex items-center gap-1 text-[11px] font-bold text-[#FF4F00] hover:text-orange-700 dark:hover:text-orange-300 transition-colors cursor-pointer"
              >
                {isContentExpanded ? (
                  <>
                    See less
        ...
        {post.image_url && (
          <div className="mt-2 mb-3 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
            <img
              src={post.image_url}
              alt={post.title}
      ...
      {/* Footer Controls */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-5 py-2.5 sm:py-3 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
        <div className="flex items-center gap-2 sm:gap-3.5 text-gray-500 dark:text-gray-400 dark:text-gray-500 font-medium text-xs sm:text-sm">
          {/* Voting Box */}
          <div
            className="flex items-center gap-1 bg-gray-50 dark:bg-gray-800/50 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg border border-gray-100 dark:border-gray-800"
            onClick={(e) => e.stopPropagation()}
          >
              <button
                type="button"
                onClick={handleUpvote}
                className={`vote-pop p-0.5 sm:p-1 rounded hover:bg-blue-50 dark:bg-blue-900/20 transition-colors cursor-pointer ${
                  voteState === 1 ? 'text-blue-500' : 'text-gray-400 dark:text-gray-500'
                }`}
                title="Upvote"
              >
              ...
              <span className={`text-[11px] sm:text-xs font-bold px-0.5 ${
                voteState === 1 ? 'text-blue-500' : voteState === -1 ? 'text-[#FF4F00]' : 'text-gray-700 dark:text-gray-300 dark:text-gray-600'
              }`}>
                {voteCount}
              </span>
              ...
              <button
                ...
                className={`vote-pop p-0.5 sm:p-1 rounded hover:bg-orange-50 dark:hover:bg-orange-900/20 dark:bg-orange-900/20 transition-colors cursor-pointer ${
                  voteState === -1 ? 'text-[#FF4F00]' : 'text-gray-400 dark:text-gray-500'
                }`}
                title="Downvote"
              >
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
          ...
          {/* Comments Link */}
          <div className="flex items-center gap-1 sm:gap-1.5 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300 dark:text-gray-600 transition-colors">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            <span className="text-[11px] sm:text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">
          ...
          <button
            type="button"
            onClick={handleCopyLink}
            className="flex items-center gap-1 sm:gap-1.5 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300 dark:text-gray-600 transition-colors cursor-pointer"
          >
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
          ...
          <button
            ...
            className={`flex items-center gap-1 sm:gap-1.5 transition-colors cursor-pointer ${
              isSaved ? 'text-[#FF4F00] font-bold' : 'text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300 dark:text-gray-600'
            }`}
          >
            <svg
        ...
        {!isOwner && post.allowMentoring && (
          <button
            ...
            className="text-[#FF4F00] bg-white dark:bg-gray-900 border border-[#FF4F00] rounded-xl px-2.5 sm:px-3.5 py-1 sm:py-1.5 text-[11px] sm:text-xs font-bold hover:bg-orange-50 dark:hover:bg-orange-900/20 dark:bg-orange-900/20 transition-colors whitespace-nowrap ml-auto sm:ml-0 cursor-pointer active:scale-95"
          >
            Request Mentoring
          </button>
        )}
