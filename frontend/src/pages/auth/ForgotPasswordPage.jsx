...
export default function ForgotPasswordPage() {
  ...
  const { resetPasswordMicrosoft, forgotPassword, resetPassword } = useAuth();
  ...
  const handleEmailReset = async (e) => {
    e.preventDefault();
    if (!isFormValid || !otpCode.trim()) return;
    setError('');
    setSending(true);
    try {
      ...
      <div className="w-full max-w-sm sm:max-w-md bg-white border border-gray-200 rounded-xl sm:rounded-2xl shadow-xs p-5 sm:p-8">
        <div className="text-center mb-5 sm:mb-6">
          ...
          <p className="text-[11px] sm:text-xs text-gray-500 mt-1 max-w-xs mx-auto leading-relaxed">
            {done ? 'Your password has been updated' : 'Choose a method to reset your password'}
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 text-xs font-bold py-2 rounded-xl transition-all cursor-pointer ${mode === 'email' ? 'bg-[#FF4F00] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
                Email Reset
              </button>
              <button
                ...
                className={`flex-1 text-xs font-bold py-2 rounded-xl transition-all cursor-pointer ${mode === 'microsoft' ? 'bg-[#FF4F00] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
                Microsoft
              </button>
            </div>
            ...
            {mode === 'email' && !otpSent && (
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div>
                  <label className="block text-[11px] font-bold text-gray-900 uppercase tracking-wider mb-1.5">Email Address</label>
                  <input
                    type="email"
                    required
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="Enter your CADT email"
                ...
                <button type="submit" disabled={sending} className="w-full bg-[#FF4F00] hover:bg-[#E64700] text-white text-xs font-bold py-2.5 sm:py-3 rounded-xl transition-all shadow-xs cursor-pointer active:scale-98 disabled:opacity-60 disabled:cursor-not-allowed">
            ...
            {mode === 'email' && otpSent && (
              <form onSubmit={handleEmailReset} className="space-y-4">
                <div>
                  <label className="block text-[11px] font-bold text-gray-900 uppercase tracking-wider mb-1.5">Reset Code</label>
                  <input
                    type="text"
                    inputMode="numeric"
                    required
                    ...
                    className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-3.5 sm:px-4 py-2.5 sm:py-3 text-xs text-center tracking-[0.3em] text-gray-900 outline-none focus:bg-white focus:border-[#FF4F00] focus:ring-1 focus:ring-[#FF4F00] transition-all shadow-2xs"
                  />
                </div>
                ...
                <button type="submit" disabled={sending || !isFormValid || otpCode.length < 6} className="w-full bg-[#FF4F00] hover:bg-[#E64700] text-white text-xs font-bold py-2.5 sm:py-3 rounded-xl transition-all shadow-xs cursor-pointer active:scale-98 disabled:opacity-60 disabled:cursor-not-allowed">
                  {sending ? 'Resetting…' : 'Reset Password'}
                </button>
                <button type="button" onClick={() => { setOtpSent(false); setOtpCode(''); setError(''); }} className="w-full text-center text-xs text-gray-500 hover:text-gray-700 font-semibold cursor-pointer">
                  Back
                </button>
              </form>
            )}

            {mode === 'microsoft' && !account && (
              <div className="space-y-4">
